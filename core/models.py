...
class ResumeRequest(models.Model):
    ...
    employers = models.TextField(verbose_name="Работодатели и должности", blank=True, default="")
    achievements = models.TextField(verbose_name="Достижения и навыки", blank=True, default="")
    images = models.ManyToManyField('UserImage', blank=True, verbose_name="Прикрепленные изображения")
    resume_content = models.TextField(verbose_name="Сгенерированное резюме (устар.)", blank=True, default="")
    resume_file = models.FileField(upload_to=resume_archive_path, null=True, blank=True, verbose_name="Файл резюме (DOCX устар.)")

    # Новые поля для мультиагентной системы и многокомпанийной генерации
    targets = models.JSONField(default=list, blank=True, help_text="Список целей: [{company, role, url}]")
    extra_instructions = models.TextField(blank=True, default="", verbose_name="Доп. инструкции")
    strict_matching = models.BooleanField(default=True, verbose_name="Строгое соответствие")
    add_skills = models.BooleanField(default=False, verbose_name="Добавлять навыки")
    specific_conditions = models.TextField(blank=True, default="", verbose_name="Специфичные условия")

    archive_file = models.FileField(upload_to=resume_archive_path, null=True, blank=True, verbose_name="Архив с резюме (ZIP)")
...
class ResumeResult(models.Model):
    request = models.ForeignKey(ResumeRequest, on_delete=models.CASCADE, related_name='results')
    company = models.CharField(max_length=255, verbose_name="Компания")
    role = models.CharField(max_length=255, blank=True, default="", verbose_name="Роль/вакансия")
    job_url = models.URLField(blank=True, default="", verbose_name="URL вакансии")

    json_data = models.JSONField(default=dict, blank=True, verbose_name="Структурированные данные по schema.json")
    json_file = models.FileField(upload_to=resume_json_path, null=True, blank=True, verbose_name="JSON файл данных")
    docx_file = models.FileField(upload_to=resume_result_file_path, null=True, blank=True, verbose_name="DOCX файл резюме")

    approved = models.BooleanField(default=False, verbose_name="Проверка пройдена")
    notes = models.TextField(blank=True, default="", verbose_name="Заметки/причина переработки")
    created_at = models.DateTimeField(auto_now_add=True)
