...
class UserProfileForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'birth_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date', 'placeholder': 'дд.мм.гггг'}),
            ...
            'skills': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Перечислите ваши навыки через запятую', 'rows': 4}),
            'additional_info': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Дополнительная информация о себе', 'rows': 4}),
...
class WorkExperienceForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Обязанности, достижения, проекты...', 'rows': 4}),
        }
        labels = {
            'company': '🏢 Компания',
            'position': '💼 Должность',
...
class EducationForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Курсы, достижения, средний балл...', 'rows': 3}),
        }
        labels = {
...
class PortfolioItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(attrs={'class': 'form-control', 'placeholder': 'Описание проекта, используемые технологии...', 'rows': 4}),
...
class UserImageForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Название изображения (опционально)'}),
        }
...
class ResumeGenerationForm(forms.Form):
    companies_text = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 6,
            'placeholder': 'Одна цель на строку в формате: Компания | Роль (опц.) | URL вакансии (опц.)'
        }),
        label="Список компаний/вакансий"
    )
    extra_instructions = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': 'Дополнительные пожелания к тону, стилю, формату...'}),
    ...
    specific_conditions = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Специфичные условия: например, без релокации, полная удаленка, готовность к командировкам...'}),
        label="Специфичные условия"
    )
    selected_images = forms.MultipleChoiceField(
        required=False,
        widget=forms.CheckboxSelectMultiple(attrs={'class': 'form-check-input'}),
        label="Прикрепить изображения из профиля"
    )

    def __init__(self, *args, **kwargs):
        user = kwargs.pop('user', None)
        super().__init__(*args, **kwargs)
        if user is not None and hasattr(user, 'images'):
            choices = [(str(img.id), img.title or f'Изображение {img.id}') for img in user.images.all()]
            self.fields['selected_images'].choices = choices

    def parse_targets(self):
        targets = []
        for line in self.cleaned_data['companies_text'].splitlines():
            raw = line.strip()
            if not raw:
                continue
